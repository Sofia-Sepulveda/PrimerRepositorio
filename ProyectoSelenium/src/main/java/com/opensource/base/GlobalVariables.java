package com.opensource.base;

public class GlobalVariables {
	
	//Variable globales, capa 3,FINAL nunca se va modificar. En mayusculas siempre los nombre de variables
	public static final String QA_URL ="https://opensource-demo.orangehrmlive.com/";
	public static final String CHROME_DRIVER ="webdriver.chrome.driver";
	public static final String PATH_CHROME_DRIVER = "./src/test/resources/Driver/Chrome89/chromedriver.exe";
	public static final String PATH_JSON_DATA ="./src/test/resources/testdata/json/";
	public static final String PATH_EXCEL_DATA = "./src/test/resources/testdata/excel/";
	public static final String PATH_SCREENSHOTS = "./test-output/screenshots/";

}
